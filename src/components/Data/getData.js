...
export default {
  ...
  downFile: function (hash) {
    ...
    Vue.http.get('/search/yy/index.php', {
      ...
      bus.$emit('setUrl', res.data.data.play_url)
    })
  },
  // 播放
    ...
    Vue.http.get('/search/yy/index.php', {
      ...
      if (control) {
        ...
        for (let i = 0; i < singer.length - 1; i++) {
          img.push('http://linkorg.oss-cn-beijing.aliyuncs.com/musicRec/addSong_2.png')
        }
      }
    })
  },
  ...
  getSingerInfo: function (singerId, c, indexs, imgs, singerLists) {
    Vue.http.get('/link/php/user-agent.php', {
      params: {
        'id': singerId
      }
    }).then((res) => {
